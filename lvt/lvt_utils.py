...
# model_type string format
# Each component: <kind>:<param>  joined by commas for stacked models.
...
#   "split_rate:4.0"
#   "abatement:100pct"
#   "split_rate:4.0,exemption:50000"
#   "abatement:50pct,exemption:70000"
...
def build_standard_export_frame(
    ...
    Returns a DataFrame indexed identically to ``df`` so callers can attach
    geometry or other per-parcel columns by index alignment.
    """
    required = [
    ...
    out['taxable_improvement_value'] = pd.to_numeric(df[taxable_improvement_col], errors='coerce').fillna(0.0)

    # Fully exempt flag
    if exempt_flag_col is not None and exempt_flag_col in df.columns:
        out['is_fully_exempt'] = (
            pd.to_numeric(df[exempt_flag_col], errors='coerce').fillna(0) != 0
        )
    else:
        # Derive: both taxable values zero AND current tax zero
        out['is_fully_exempt'] = (
            (out['taxable_land_value'] == 0) &
            (out['taxable_improvement_value'] == 0) &
            (out['current_tax'] == 0)
        )

    # Demographics — include as null if column absent
    for out_col, src_col in [
        ('std_geoid', geoid_col),
        ('median_income', income_col),
        ('minority_pct', minority_col),
        ('black_pct', black_col),
    ]:
    ...
    # Model provenance — row-constant
    out['model_type'] = model_type
    out['land_millage'] = float(land_millage)
    out['improvement_millage'] = float(improvement_millage)

    # Validation: non-standard property categories (warn only)
    cats = set(out['property_category'].unique())
    nonstandard = cats - STANDARD_PROPERTY_CATEGORIES
    if nonstandard:
        print(
            f"  [warn] {city}: non-standard property categories "
            f"(will be preserved): {sorted(nonstandard)}"
        )

    # Validation: revenue neutrality (warn only)
    total_current = out['current_tax'].sum()
    total_new = out['new_tax'].sum()
    if total_current > 0:
        rev_diff_pct = abs(total_new - total_current) / total_current * 100
        if rev_diff_pct > 1.0:
            print(
                f"  [warn] {city}: revenue not neutral — "
...
def save_standard_export(
    ...
    land_millage : float
        Effective land millage rate (per $1,000) from the model.
    improvement_millage : float
        Effective improvement millage rate (per $1,000) from the model.
    property_category_col : str
        Column in df holding the property category strings.
    current_tax_col : str
        Column holding the current (pre-LVT) tax per parcel.
    new_tax_col : str
    ...
    taxable_improvement_col : str
        Column holding post-exemption taxable improvement value.
    exempt_flag_col : str, optional
        Column holding a boolean/binary fully-exempt flag. If None, is_fully_exempt
        is derived as True where both taxable values are zero and current_tax is zero.
    geoid_col : str
        Column holding 12-digit Census block group GEOID (joined from census step).
    income_col : str
        Column holding block-group median household income.
    minority_col : str
    ...
    print(
        f"  ✓ {city}: {len(out):,} rows → {output_path}  "
        f"[model: {model_type}]"
    )
