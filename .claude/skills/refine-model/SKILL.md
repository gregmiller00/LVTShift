...
1. **Surgical edits only.** Change the specific constants/cells the refinement requires and
   ...
   closing census-join → `save_standard_export` → `create_city_report` pattern (it must
   stay verbatim per `build-notebook.md`).
...
- If the change alters which variable holds the millages (split-rate → abatement), update
  ...
  structure intact.
- Do **not** alter the census-join / export / report cells beyond required variable names.
